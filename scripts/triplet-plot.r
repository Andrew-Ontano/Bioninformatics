...
rc = function(sequence){
  seq = strsplit(sequence,"")[[1]]
  map=c("A"="T", "T"="A","G"="C","C"="G")
  return(paste0(rev(unname(map[seq])),collapse=""))
}

simplify = function(triplet){
  return(sort(c(paste(strsplit(triplet,"")[[1]],collapse=''),paste(strsplit(triplet,"")[[1]][c(2,3,1)],collapse=''), paste(strsplit(triplet,"")[[1]][c(3,1,2)],collapse='')))[1])
}
...
if(is.null(opt$input_filename)){
  cat("No tsv specified. See usage \"with triplet-plot.r -h\"")
  quit()
}else{
...
if(!is.null(opt$sequences)){
  values = values %>% filter(Scaffold%in%as.vector(opt$sequences))
}
...
height_factor = length(table(values$Scaffold))
...
if(height_factor>1){
  p = p + facet_grid(rows = vars(values$Scaffold))
}
...
if(!is.null(opt$gff) && !is.null(opt$sequences)){
  ...
  names(genes) = c("Scaffold","Source","Type","Start","End","t1","Direction","t2","Info")
